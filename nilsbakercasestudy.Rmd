...
The p-value of `Footprint` is relatively low at 0.063 (see `lm_9` in __Codeblock 8__ of the _Appendix_ for the full summary of the model). While not below the standard $\alpha = 0.05$, it is very close and the interaction term with `Footprint` is extremely significant with a p-value of 1.48e-12. The other models that we tried tended to show this pattern as well (see __Codeblock 8__ in the _Appendix_ for the full summary of each model). That is, `Footprint` is not insignificant, but its real value is in its interaction terms. This plays a key role in determinig if having a physical branch motivates people to open checking accounts with the bank.  
